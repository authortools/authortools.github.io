document.addEventListener('DOMContentLoaded', () => {
    ...
    const dom = {
        ...
        soundKeypressToggle: document.getElementById('sound-keypress-toggle'),
        soundWarningToggle: document.getElementById('sound-warning-toggle'),
        keypressSound: document.getElementById('keypress-sound'),
        ...
        draftsModal: document.getElementById('drafts-modal'),
        closeDraftsModalButton: document.getElementById('close-drafts-modal-button'),
        draftsList: document.getElementById('drafts-list'),
        newDraftNameInput: document.getElementById('new-draft-name'),
        addNewDraftButton: document.getElementById('add-new-draft-button'),
    };
    ...
    let state = {
        ...
        soundKeypress: false, soundWarning: false,
    ...
    // =================================================================================
    // 5. ЛОГИКА ЧЕРНОВИКОВ (DRAFTS)
    // =================================================================================
    const saveDraftsToStorage = () => localStorage.setItem('writer-app-drafts', JSON.stringify(state.drafts));
    const saveActiveDraftIdToStorage = () => localStorage.setItem('writer-app-active-draft', state.activeDraftId);
        ...
        state.drafts.forEach(draft => {
            ...
            li.innerHTML = `
                ...
                <div class="draft-actions">
                    <button class="btn btn-secondary load-draft-btn" data-id="${draft.id}">${LANG_DATA[state.currentLang].draft_load_btn}</button>
                    <button class="btn btn-danger delete-draft-btn" data-id="${draft.id}">${LANG_DATA[state.currentLang].draft_delete_btn}</button>
                </div>
            `;
            dom.draftsList.appendChild(li);
        });
    };
    ...
    const loadDraft = (id) => {
        ...
        if (draft) {
            ...
            dom.editor.innerHTML = draft.content;
            updateStats();
            saveActiveDraftIdToStorage();
        }
    ...
    const addNewDraft = () => {
        const name = dom.newDraftNameInput.value.trim();
        if (!name) {
            alert('Введите имя черновика!');
            return;
        }
        const newDraft = {
            id: `draft-${Date.now()}`,
    ...
    const initializeDrafts = () => {
        const storedDrafts = localStorage.getItem('writer-app-drafts');
        if (storedDrafts) {
            state.drafts = JSON.parse(storedDrafts);
        } else {
            state.drafts = [{ id: 'default', name: 'Основной черновик', content: '' }];
            saveDraftsToStorage();
        }
        const storedActiveId = localStorage.getItem('writer-app-active-draft');
        state.activeDraftId = storedActiveId && state.drafts.some(d => d.id === storedActiveId) ? storedActiveId : state.drafts[0].id;
        loadDraft(state.activeDraftId);
        renderDraftsList();
    };
    ...
    const autoSaveCurrentDraft = () => {
        ...
        if (activeDraft) {
            activeDraft.content = dom.editor.innerHTML;
            saveDraftsToStorage();
        }
    };
    ...
    // =================================================================================
    const playSound = (soundElement) => { soundElement.currentTime = 0; soundElement.play().catch(e => console.log("Playback was interrupted.")); };
    ...
    const debouncedSaveAndStats = debounce(() => { updateStats(); autoSaveCurrentDraft(); }, 300);
    ...
    const armPunishmentTimer = () => {
        ...
        if (state.soundWarning && waitTime > 1000) {
            clearTimeout(state.warningTimer);
            state.warningTimer = setTimeout(() => playSound(dom.warningSound), waitTime - 1000);
        }
    ...
    const handlePunishmentChange = () => { /* ... (без изменений) ... */ const type = dom.punishmentSelect.value; const showWait = type !== 'hardcore'; const showSpeed = type === 'delete' || type === 'hardcore' || type === 'impossible'; dom.waitTimeGroup.classList.toggle('hidden', !showWait); dom.deleteSpeedGroup.classList.toggle('hidden', !showSpeed); };
    const handleModeChange = () => { /* ... (без изменений) ... */ const mode = dom.modeSelect.value; dom.sprintSettings.classList.toggle('hidden', mode !== 'sprint'); dom.driveSettings.classList.toggle('hidden', mode !== 'drive'); if (state.isWritingSessionLive) stopCurrentSession(); };
    const applyTheme = (theme) => { /* ... (без изменений) ... */ if (theme === 'dark') { dom.html.classList.add('dark-theme'); dom.themeSwitcher.textContent = '☀️'; } else { dom.html.classList.remove('dark-theme'); dom.themeSwitcher.textContent = '🌙'; }};
    const applyLanguage = (lang) => { /* ... (без изменений) ... */ state.currentLang = lang; document.querySelectorAll('[data-lang-key]').forEach(el => { const key = el.dataset.langKey; const translation = LANG_DATA[lang][key]; if(translation) { if (el.hasAttribute('placeholder')) { el.setAttribute('placeholder', translation); } else { el.textContent = translation; } } }); renderDraftsList(); };
    ...
    dom.editor.addEventListener('input', () => {
        ...
        if (state.soundKeypress) playSound(dom.keypressSound);
    ...
    dom.editor.addEventListener('keydown', (e) => { /* ... (без изменений) ... */ const type = dom.punishmentSelect.value; const isHardcore = (type === 'hardcore' || type === 'impossible') && state.isWritingSessionLive; if (isHardcore && (e.key === 'Backspace' || e.key === 'Delete')) e.preventDefault(); });
    ...
    dom.saveButton.addEventListener('click', () => { const blob = new Blob([dom.editor.textContent], { type: 'text/plain;charset=utf-8' }); const link = document.createElement('a'); const draftName = state.drafts.find(d => d.id === state.activeDraftId)?.name || 'текст'; link.href = URL.createObjectURL(blob); link.download = `${draftName}-${new Date().toISOString().slice(0, 10)}.txt`; link.click(); });
    dom.copyEditorButton.addEventListener('click', () => { /* ... (без изменений) ... */ const textToCopy = dom.editor.textContent; if (!textToCopy) return; navigator.clipboard.writeText(textToCopy).then(() => { dom.copyEditorButton.textContent = LANG_DATA[state.currentLang].btn_copied; dom.copyEditorButton.disabled = true; setTimeout(() => { dom.copyEditorButton.textContent = LANG_DATA[state.currentLang].btn_copy; dom.copyEditorButton.disabled = false; }, 1500); }).catch(err => alert('Не удалось скопировать текст.')); });
    dom.ideaButton.addEventListener('click', () => { /* ... (без изменений) ... */ state.isProgrammaticChange = true; let lastElement = dom.editor.lastElementChild; let newIdea; let lastKnownIdeaText = (lastElement && lastElement.classList.contains('suggested-idea')) ? lastElement.textContent : null; do { newIdea = IDEAS[state.currentLang][Math.floor(Math.random() * IDEAS[state.currentLang].length)]; } while (newIdea === lastKnownIdeaText && IDEAS[state.currentLang].length > 1); if (lastElement && lastElement.classList.contains('suggested-idea')) { lastElement.textContent = newIdea; } else { const newIdeaSpan = document.createElement('span'); newIdeaSpan.className = 'suggested-idea'; newIdeaSpan.textContent = newIdea; if (dom.editor.innerHTML.trim() !== '' && !dom.editor.innerHTML.trim().endsWith('<br>')) { dom.editor.append(document.createElement('br'), document.createElement('br')); } dom.editor.append(newIdeaSpan); } debouncedSaveAndStats(); setCaretAtEnd(); state.isProgrammaticChange = false; });
    
    // --- Обработчики новых настроек ---
    dom.soundKeypressToggle.addEventListener('change', (e) => { state.soundKeypress = e.target.checked; localStorage.setItem('writer-app-sound-keypress', state.soundKeypress); });
    dom.soundWarningToggle.addEventListener('change', (e) => { state.soundWarning = e.target.checked; localStorage.setItem('writer-app-sound-warning', state.soundWarning); });
    ...
    dom.draftsButton.addEventListener('click', () => dom.draftsModal.classList.remove('hidden'));
    dom.closeDraftsModalButton.addEventListener('click', () => dom.draftsModal.classList.add('hidden'));
    dom.addNewDraftButton.addEventListener('click', addNewDraft);
    dom.draftsList.addEventListener('click', (e) => {
        const target = e.target;
        if (target.classList.contains('load-draft-btn')) loadDraft(target.dataset.id);
        if (target.classList.contains('delete-draft-btn')) deleteDraft(target.dataset.id);
    });
    ...
    // =================================================================================
    const initialize = () => {
        initializeDrafts(); // Загружаем черновики
        handleModeChange();
        ...
        state.soundKeypress = localStorage.getItem('writer-app-sound-keypress') === 'true';
        state.soundWarning = localStorage.getItem('writer-app-sound-warning') === 'true';

        dom.soundKeypressToggle.checked = state.soundKeypress;
        dom.soundWarningToggle.checked = state.soundWarning;

        applyTheme(savedTheme);
        applyLanguage(savedLang); // Язык должен применяться последним, чтобы перерисовать UI

        // Изначально сворачиваем настройки
